...
const ProjectExperienceItem: React.FC<Props> = ({
  data,
  index,
  totalLength,
  onEditButton,
  ...
  return (
    <>
      <Card className="bg-light mt-0" key={index}>
        ...
        <Card.Text>{data.projectWorker}</Card.Text>
        <Card.Text>
          {convertDate(new Date(data.projectStartDate))} s/d{" "}
          {data.stillWorkingInThisProject
            ? "sekarang"
            : convertDate(new Date(data.projectEndDate))}
        </Card.Text>
